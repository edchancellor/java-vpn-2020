...
public class ForwardServerClientThread extends Thread
{
     ...
     */
    public ForwardServerClientThread(boolean id, ServerSocket listensocket, String serverhost, int serverport, SessionEncrypter se, SessionDecrypter sd) throws IOException
    {
        mListenSocket = listensocket;
        mServerPort = serverport;
        mServerHost = serverhost;
        session_encrypter = se;
        session_decrypter = sd;
        am_I_Client = id;
    }
    ...
    public void run()
    {
        try {
            ...
            //Encrypter and Decrypter ADDED
            if (am_I_Client) 
            {
               clientIn = session_decrypter.openCipherInputStream(clientIn);
               clientOut = session_encrypter.openCipherOutputStream(clientOut);
            } 
            else 
            {
               serverOut = session_encrypter.openCipherOutputStream(serverOut);
               serverIn = session_decrypter.openCipherInputStream(serverIn);
            }
